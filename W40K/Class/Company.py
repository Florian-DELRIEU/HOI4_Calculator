...
class Company:
    ...
    def HOI4_Profil(self):
        if self.Unit is None:
            ...
            self.Defense = self.Unit.Defense * np.sum([el.Defense for el in self.Equipement]) / 10
            self.Breakthrought = self.Unit.Breakthrought * np.sum([el.Breakthrought for el in self.Equipement]) / 10
